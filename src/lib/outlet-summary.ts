...
import { getAllOutlets } from './outlets-registry'

// Ringkasan lintas outlet (omzet, transaksi, 7 hari, P&L bulan berjalan).
// Dipakai bersama oleh /api/owner/summary (halaman /owner, password owner) dan
// /api/central/summary (tab "Pantau Outlet" di Admin Pusat, password admin) —
// satu sumber perhitungan, biar angkanya tidak pernah beda antar halaman.
...
export type OutletSummaryErr = { name: string; ok: false; error: string }
export type SummaryResult = {
  today: string
  month: string
  outlets: (OutletSummaryOk | OutletSummaryErr)[]
  aggregate: {
...
export async function buildOutletSummary(): Promise<SummaryResult | { empty: true }> {
  const outlets = await getAllOutlets()
  if (!outlets.length) return { empty: true }
  ...
  const okOutlets = outletData.filter((o) => o.ok) as OutletSummaryOk[]
  const aggregate = {
  ...
  return { today, month, outlets: outletData, aggregate }
}
